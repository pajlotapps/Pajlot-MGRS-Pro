...
class DMStoMGRS: UIViewController, UITextFieldDelegate {
    ...
    func updateView() {
        setPlaceholders()
        tfCollection.forEach { (textfield) in
    ...
    func MGRSString (Lat: Double, Long: Double) -> String {
        ...
        if (Lat < -80) {
            ...
            let afString = e100kLetters[Int(afIndex)]
            ...
            let ahString = n100kLetters[Int(ahIndex)]
    ...
    @objc func textFieldDidChange(textField: UITextField) {
        ...
        case latDDTF:
            if textField.text?.utf16.count == 2 {
                latMMTF.becomeFirstResponder()
            }
        case latMMTF:
            if textField.text?.utf16.count == 2 {
                latSSTF.becomeFirstResponder()
            }
        case latSSTF:
            if textField.text?.utf16.count == 2 {
                longDDTF.becomeFirstResponder()
            }
        case longDDTF:
            if textField.text?.utf16.count == 3 {
                longMMTF.becomeFirstResponder()
            }
        case longMMTF:
            if textField.text?.utf16.count == 2 {
                longSSTF.becomeFirstResponder()
            }
        case longSSTF:
            if textField.text?.utf16.count == 2 {
